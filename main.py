...
cursor.execute("CREATE TABLE IF NOT EXISTS data (Name TEXT UNIQUE, Quantity INTEGER, Minimum INTEGER, Price INTEGER)")
...
class MainWeb(tornado.web.RequestHandler):
    ...
    def get(self):
        ...
        if isinstance(name, str):
            ...
            if data == "q":
                ...
                cursor.execute("UPDATE data SET Quantity=? WHERE Name=?", (quantity, name))

            if data == "m":
                quantity = int(quantity)

                cursor.execute("SELECT Minimum FROM data WHERE Name=?", (name,))

                quantity_2 = cursor.fetchall()

                quantity = quantity + int(quantity_2[0][0])

                cursor.execute("UPDATE data SET Minimum=? WHERE Name=?", (quantity, name))
    ...
    def post(self):
        ...
        try:
            cursor.execute("INSERT INTO data(Name, Quantity, Minimum) VALUES(?, ?, ?)", (name, quantity, minimum))

        except:
            cursor.execute("UPDATE data SET Quantity=?, Minimum=? WHERE Name=?", (quantity, minimum, name))
